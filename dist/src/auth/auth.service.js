...
let AuthService = class AuthService {
    ...
    async register(email, username, password) {
        ...
        if (existingEmail != null) {
            throw new common_1.UnauthorizedException("Bu email allaqachon ro'yxatdan o'tgan");
        }
        const existingUsername = await this.prisma.user.findUnique({
            where: { username },
        });
        if (existingUsername) {
            throw new common_1.UnauthorizedException('Bu username allaqachon band');
        }
    ...
    async login(email, password) {
        ...
        if (!user) {
            throw new common_1.NotFoundException('Bunday email bilan foydalanuvchi topilmadi');
        }
